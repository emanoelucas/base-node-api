...
describe('Request body validator', () => {
  it('should throw if requiredParams is not an array', () => {
    ...
    const receivedData = {}
    expect(() => { requestBodyValidator.validate(requiredParams, receivedData) }).toThrow(new InvalidParamError('requiredParams is not an array'))
	})
  it('should throw if no requiredParams are provided', () => {
		const requiredParams: any = []
    const receivedData = {}
    expect(() => { requestBodyValidator.validate(requiredParams, receivedData) }).toThrow(new MissingParamError('requiredParams'))
	})
  it('should throw if no receivedData are provided', () => {
		const requiredParams: any = ['field1', 'field2']
    const receivedData: any = ''
    expect(() => { requestBodyValidator.validate(requiredParams, receivedData) }).toThrow(new MissingParamError('receivedData'))
	})
  it('should proceed if receivedData match all requiredParams', () => {
		const requiredParams: any = ['field1', 'field2']
    const receivedData: any = {field1: 'value1', field2: 'value2'}
    expect(requestBodyValidator.validate(requiredParams, receivedData)).toBeUndefined()
	})
  it('should throw if receivedData does not match the requiredParams', () => {
		const requiredParams: any = ['field1', 'field2']
    const receivedData: any = {field1: 'value1', field3: 'value3'}
    expect(() => { requestBodyValidator.validate(requiredParams, receivedData) }).toThrow(BadRequestError)
	})
})